...
// Workload object shape. `this` is typed so the source/src/workloads methods can
// read this.N / this.source without tripping noImplicitAny (TS2683); the index
// signature absorbs the heterogeneous extras (`source`/`src`/`label`) and the
// per-case shapes (gate/run/batch/reps/keep/label) stay `any` — they are
// structurally diverse by design and carry no type-level invariant worth pinning.
interface Spec {
  N: number
  workloads(n?: number): Record<string, any>
  [k: string]: any
}

export const filter: Spec = {
  N: 10_000,
  source(this: any, n = this.N) {
    const o: any = {}
  ...
  workloads(this: any, n = this.N) {
    const mk = () => {
      const s: any = $(this.source(n))
      const f = s.filter((d: any) => d.active) // hold f so the chain stays alive
      return { s, f }
    }
    const single = mk()
    const batch = mk()
    ...
    const aS: any = $(this.source(n)); const ab: any = $(true); const aV = aS.filter('active', ab)
    return {
      // setup rebuilds a fresh source+filter each rep — already rep-idempotent
      setup: { gate: 500, run: () => { const s: any = $(this.source(n)); s.filter((d: any) => d.active) } },
...
export const map: Spec = {
  ...
  workloads(this: any, n = this.N) {
    const mk = () => { const s: any = $(this.source(n)); const m = s.map((d: any) => d.x + d.y); return { s, m } }
    const ins = mk(); let i = n
    return {
      setup: { gate: 500, run: () => { const s: any = $(this.source(n)); s.map((d: any) => d.x + d.y) } },
      insert: { gate: 50, keep: ins.m, run: () => { ins.s.insert({ x: i, y: i * 2 }); i++ } },
    }
  },
}

export const to: Spec = {
  N: 10_000,
  label: 'to',
  source(this: any, n = this.N) {
    const o: any = {}
  ...
  workloads(this: any, n = this.N) {
    ...
    const mk = () => { const s: any = $(this.source(n)); const t = s.to(proj); return { s, t } }
    const ins = mk(); const bat = mk(); let i = n; let toggle = false
    return {
      setup: { gate: 500, run: () => { const s: any = $(this.source(n)); s.to(proj) } },
...
export const length: Spec = {
  ...
  workloads(this: any, n = this.N) {
    const mk = () => { const s: any = $(this.source(n)); const c = s.length(); return { s, c } }
    const ins = mk(); let i = n
    return {
...
export const lengthFn: Spec = {
  ...
  workloads(this: any, n = this.N) {
    return {
      ...
      setup: { gate: 500, run: () => { const s: any = $(this.source(n)); s.length((d: any) => d.bucket) } },
    }
  },
}
...
export const keys: Spec = {
  ...
  workloads(this: any, n = this.N) {
    const mk = () => { const s: any = $(this.source(n)); const k = s.keys(); return { s, k } }
    const ins = mk(); let i = n
    return {
      setup: { gate: 500, run: () => { const s: any = $(this.source(n)); s.keys() } },
      insert: { gate: 50, keep: ins.k, run: () => { ins.s.insert({ active: true, val: i++ }) } },
    }
  },
}

export const values: Spec = {
  N: 10_000,
  label: 'values',
  source: keys.source,
  workloads(this: any, n = this.N) {
    const mk = () => { const s: any = $(this.source(n)); const v = s.values(); return { s, v } }
    const bat = mk(); let toggle = false
    return {
      setup: { gate: 500, run: () => { const s: any = $(this.source(n)); s.values() } },
...
export const tap: Spec = {
  N: 10_000,
  label: 'tap',
  source: keys.source,
  workloads(this: any, n = this.N) {
    const mkFull = () => { const s: any = $(this.source(n)); let c = 0; const t = s.tap((ch: any) => { c++ }); return { s, t } }   // 1-arg → full TapValue (clones)
    const mkBare = () => { const s: any = $(this.source(n)); let c = 0; const t = s.tap(() => { c++ }); return { s, t } }   // 0-arg → TapBareValue (no clone)
    const ins = mkFull(); const bat = mkFull(); const bare = mkBare(); let i = n; let tb = false; let tbare = false
    return {
      setup: { gate: 1000, run: () => { const s: any = $(this.source(n)); let c = 0; s.tap(() => { c++ }) } },
      insert: { gate: 50, keep: ins.t, run: () => { ins.s.insert({ active: true, val: i++ }) } },
...
export const reverse: Spec = {
  N: 10_000,
  label: 'reverse',
  source: keys.source,
  workloads(this: any, n = this.N) {
    const mk = () => { const s: any = $(this.source(n)); const r = s.reverse(); return { s, r } }
    const ins = mk(); const bat = mk(); let i = n; let toggle = false
    return {
      setup: { gate: 500, run: () => { const s: any = $(this.source(n)); s.reverse() } },
...
export const distinct: Spec = {
  ...
  workloads(this: any, n = this.N) {
    const mk = () => { const s: any = $(this.source(n)); const d = s.distinct((r: any) => r.cat); return { s, d } }
    const ins = mk(); const bat = mk(); let i = n; let toggle = false
    return {
      setup: { gate: 500, run: () => { const s: any = $(this.source(n)); s.distinct((r: any) => r.cat) } },
...
export const compare: Spec = {
  ...
  workloads(this: any, n = this.N) {
    const mkGt = () => { const s: any = $(this.source(n)); const f = s.gt('val', 5000); return { s, f } }
    ...
    const tS: any = $(this.source(n)); const tb: any = $(5000); const tV = tS.gt('val', tb)
    return {
      'gt-setup': { gate: 500, run: () => { const s: any = $(this.source(n)); s.gt('val', 5000) } },
      ...
      'lt-setup': { gate: 500, run: () => { const s: any = $(this.source(n)); s.lt('val', 5000) } },
      'gte-setup': { gate: 500, run: () => { const s: any = $(this.source(n)); s.gte('val', 5000) } },
      'lte-setup': { gate: 500, run: () => { const s: any = $(this.source(n)); s.lte('val', 5000) } },
    }
  },
}

export const between: Spec = {
  N: 10_000,
  label: 'between',
  source(this: any, n = this.N) {
    const o: any = {}
  ...
  workloads(this: any, n = this.N) {
    ...
    const nS: any = $(this.source(n)); const nb: any = $({ lo: 0, hi: 1000 }); const nV = nS.between('val', [nb.lo, nb.hi])
    // insert churn (object): src.insert grows the source; reps=3 (consumes)
    const iS: any = $(this.source(n)); const ib: any = $({ lo: 200, hi: 800 }); const iV = iS.between('val', [ib.lo, ib.hi]); let id = 100000
    // remove churn (object): delete distinct keys per rep; reps=5 (consumes 5×1000 of 10000)
    const rS: any = $(this.source(n)); const rb: any = $({ lo: 200, hi: 800 }); const rV = rS.between('val', [rb.lo, rb.hi]); let base = 0
    return {
      setup: { gate: 500, run: () => { const s: any = $(this.source(n)); const b: any = $({ lo: 200, hi: 800 }); s.between('val', [b.lo, b.hi]) } },
...
export const sort: Spec = {
  ...
  workloads(this: any, n = this.N) {
    ...
    const iS: any = $(this.source(n)); const iV = iS.za('score', 100); let i = n
    // rotate: bump the row at the window tail (rank 99, lowest) past rank 0
    const rS: any = $(this.source(n)); const rV = rS.za('score', 100)
    const lastId = rV[value][rV[value].length - 1].id
    let bump = Math.max(...rV[value].map((r: any) => r.score)) + 1
    // brush: bounded top-100 over a between, brush the whole window out then in
    const bo: any = {}; for (let k = 0; k < n; k++) bo['v' + k] = { r: Math.random(), id: k }
    const bS: any = $(bo); const bB: any = $([0, 1]); const bV = bS.between('r', bB).za('r', 100)
    ...
    const wS: any = $(this.source(n)); const wn: any = $(50); const wV = wS.za('score', wn)
    return {
      setup: { gate: 500, run: () => { const s: any = $(this.source(n)); s.za('score', 100) } },
...
export const aggregate: Spec = {
  ...
  workloads(this: any, n = this.N) {
    const mk = (build: any) => { const s: any = $(this.source(n)); const a = build(s); return { s, a } }
    const sumIns = mk((s: any) => s.sum('val')); const maxIns = mk((s: any) => s.max('val'))
    const avgBat = mk((s: any) => s.avg('val')); const minBat = mk((s: any) => s.min('val')); const everyBat = mk((s: any) => s.every((r: any) => r.active))
    ...
    const cO: any = {}; for (let k = 0; k < n; k++) cO[k] = { a: k, b: k * 2 }
    const cS: any = $(cO); const cCol: any = $('a'); const cV = cS.sum(cCol)
    return {
      'sum-setup': { gate: 500, run: () => { const s: any = $(this.source(n)); s.sum('val') } },
      ...
      'max-setup': { gate: 500, run: () => { const s: any = $(this.source(n)); s.max('val') } },
      ...
      'some-setup': { gate: 500, run: () => { const s: any = $(this.source(n)); s.some((r: any) => r.active) } },
...
export const union: Spec = {
  N: 10_000,
  label: 'union',
  src(start: any, n: any) { const o: any = {}; for (let i = 0; i < n; i++) o[start + i] = `v${start + i}`; return o },
  workloads(this: any) {
    const N = this.N
    const build = () => { const a: any = $(this.src(0, N)); const b: any = $(this.src(5000, N)); const c: any = $(this.src(10000, N)); const u = a.union(b, c); return { a, b, c, u } }
    const churnG = build(); const insG = build(); let ins = 25000
    return {
      setup: { gate: 500, run: () => { const a: any = $(this.src(0, N)); const b: any = $(this.src(5000, N)); const c: any = $(this.src(10000, N)); a.union(b, c) } },
...
export const intersect: Spec = {
  N: 10_000,
  label: 'intersect',
  src(n: any) { const o: any = {}; for (let i = 0; i < n; i++) o[i] = `v${i}`; return o },
  workloads(this: any) {
    const N = this.N
    const build = () => { const a: any = $(this.src(N)); const b: any = $(this.src(8000)); const c: any = $(this.src(6000)); const x = a.intersect(b, c); return { a, b, c, x } }
    const churnG = build()
    return {
      setup: { gate: 500, run: () => { const a: any = $(this.src(N)); const b: any = $(this.src(8000)); const c: any = $(this.src(6000)); a.intersect(b, c) } },
      churn: { gate: 200, batch: 1000, keep: churnG, run: () => { for (let i = 0; i < 1000; i++) delete churnG.b[i]; for (let i = 0; i < 1000; i++) churnG.b[i] = `v${i}` } },
    }
  },
}

export const except: Spec = {
  N: 10_000,
  label: 'except',
  src(start: any, n: any) { const o: any = {}; for (let i = 0; i < n; i++) o[start + i] = `v${start + i}`; return o },
  workloads(this: any) {
    const N = this.N
    const insG = (() => { const a: any = $(this.src(0, N)); const b: any = $(this.src(0, 5000)); const x = a.except(b); return { a, b, x } })()
    ...
    const remG = (() => { const a: any = $(this.src(0, N)); const b: any = $(this.src(0, 6000)); const x = a.except(b); return { a, b, x } })()
    let ins = 5000; let base = 0
    return {
      setup: { gate: 500, run: () => { const a: any = $(this.src(0, N)); const b: any = $(this.src(0, 5000)); a.except(b) } },
...
export const reduce: Spec = {
  ...
  workloads(this: any, n = this.N) {
    ...
    const mkFull = () => { const s: any = $(this.source(n)); const a = s.reduce(add, 0); return { s, a } }                  // general fold, O(n)/event
    const mkInc = () => { const s: any = $(this.source(n)); const a = s.reduce(add, sub, 0); return { s, a } }              // incremental, O(Δ)/event
    ...
    return {
      setup: { gate: 500, run: () => { const s: any = $(this.source(n)); s.reduce(add, 0) } },
      ...
      'inc-setup': { gate: 500, run: () => { const s: any = $(this.source(n)); s.reduce(add, sub, 0) } },
