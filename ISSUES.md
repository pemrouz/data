...
|---|---|---|---|---|
| [C12](#c12) | set-algebra producers (`intersect`/`union`/`except`) desync over an **array** source under churn (C1-family array-positional gap); object shapes fixed; **`intersect` + `union` fixed**, only `except` array parked in `KNOWN_FAILURES` | Correctness | Low | Open (not shipped-reachable; object + intersect/union-array fixed) |
...
### C12
**set-algebra producers (`intersect`/`union`/`except`) desync over an ARRAY source under churn** · Low · Open (`intersect` + `union` fixed; only `except` array remains) (not shipped-reachable, object shapes fixed)

`intersect`/`union`/`except` over an **array** source drift under insert / remove / in-place-edit churn — a C1-family array-positional gap: their bitmask/`view.value` isn't fully maintained across array splices and hole accumulation (e.g. `intersect` accumulated a **ghost row** over a remove sequence; `intersect2` desynced on the first array insert; `union`/`except` array desync on an in-place edit). The harness **covers** the family: five scenarios — `intersect [array]`, `intersect2 [array]`, `intersect-between [array]`, `union [array]`, `except [array]` — were parked in `KNOWN_FAILURES` in [differential.test.ts](differential.test.ts) (the registry **fails if one starts passing**, so a fix must delete it). The **OBJECT shapes are all correct** — the only object bug (`except` re-materialising a row pushed into the exclusion by an in-place edit, because its `BU2` ignored exclusion membership) is **fixed** (see [DECISIONS.md → C12 (object half)](DECISIONS.md)).

**Progress: the `intersect` family AND `union` are FIXED** (the three `intersect*/array` cases plus `union [array]` now pass and are deleted from `KNOWN_FAILURES`) via array-only `BI0A`/`BR1A` handlers — see [DECISIONS.md → C12 (array half — intersect)](DECISIONS.md) and [→ C12 (array half — union)](DECISIONS.md). **Only `except [array]` remains parked.**
...
- Where: ~~[operators/intersect/index.ts](operators/intersect/index.ts)~~ (FIXED), ~~[operators/union/index.ts](operators/union/index.ts)~~ (FIXED), [operators/except/index.ts](operators/except/index.ts) (array-source `_enter`/`_leave` + `view.value` maintenance across array splice/hole churn; needs `BI0A`/`BR1A` handlers like the C1 family).
- Fix direction (for the remaining `except`): mirror the intersect/union fix — a STRUCTURAL array remove splices `view.value` only on the **primary** echo (`v === this.p`), routing a secondary's removal to the by-name membership path; a tail insert grows the slot naturally and decides visibility from each source's carried value (a hole, `undefined`, is NOT membership). For `except` the membership rule is "in primary AND NOT in other"; mind that `other`'s structural remove (BR1A) is a no-op echo of the underlying delete, while `other`'s membership change is a BH1/BF0 (already wired). Order-independent, no cross-source length compare. Delete `except [array]` from `KNOWN_FAILURES` once green; gate on the crossfilter/swarm/library Playwright specs.
- **Latent (separate, not C12): `RowOperator` (filter/map/compare) over an ARRAY whose TRAILING rows are excluded has `.length` < source**, so a tail insert past that length `splice`s at the wrong index and the filtered view's positions desync from the source. `union`'s fix sidesteps it by using the echo's CARRIED value rather than re-reading the source array positionally; a root fix (pad the `RowOperator` array output to source length) is a broader change — it alters the emitted array shape (the `compare - array source with delete propagates shift` test asserts the un-padded form) and belongs in its own commit, not C12.
