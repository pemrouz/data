...
|---|---|---|---|---|
| [C12](#c12) | set-algebra producers (`intersect`/`union`/`except`) desync over an **array** source under churn (C1-family array-positional gap); object shapes fixed; **`intersect` family fixed**, `union`/`except` array parked in `KNOWN_FAILURES` | Correctness | Low | Open (not shipped-reachable; object + intersect-array fixed) |
...
### C12
**set-algebra producers (`intersect`/`union`/`except`) desync over an ARRAY source under churn** · Low · Open (intersect family fixed; `union`/`except` array remain) (not shipped-reachable, object shapes fixed)
...
**Progress: the `intersect` family is FIXED** (the three `intersect*/array` cases now pass and are deleted from `KNOWN_FAILURES`) via array-only `BI0A`/`BR1A` handlers — see [DECISIONS.md → C12 (array half — intersect)](DECISIONS.md). **`union [array]` and `except [array]` remain parked.**
...
- Where: ~~[operators/intersect/index.ts](operators/intersect/index.ts)~~ (FIXED), [operators/union/index.ts](operators/union/index.ts), [operators/except/index.ts](operators/except/index.ts) (array-source `_enter`/`_leave` bitmask + `view.value` maintenance across array splice/hole churn; need `BI0A`/`BR1A` handlers like the C1 family).
- Fix direction (for the remaining `union`/`except`): mirror the intersect fix — a STRUCTURAL array remove splices `view.value` (and any bitmask) only on the **primary** echo (`v === this.p`), routing a secondary's removal to the by-name membership path; a tail insert grows the slot naturally and folds each source's membership in from its own carried value (a hole, `undefined`, must NOT count as membership). Order-independent, no cross-source length compare (a filter source's trailing holes make its `.length` unreliable). NB `union`'s primary echoes FIRST (its `p` is itself a derived facet), unlike intersect/except whose primary echoes last — the handler must not depend on order. Delete each scenario from `KNOWN_FAILURES` as it goes green; gate on the crossfilter/swarm/library Playwright specs (the shipped object/static paths must stay correct).
