...
**Fixed & committed:** C7 (`distinct` representative rebucket), D1 (`connect(fn)` fail-fast), C5 (opt-in `$.debug` reduce-symmetry check), C6 (docs), **C2** (`RowOperator.BI0A`), the `group`-over-sparse-object crash, the full **C1 array-positional protocol surgery**, and now **C3** (chained windowed sort — content-stable rotation emission). **All 9** array-positional permutations closed; `KNOWN_FAILURES` is now empty.

**Proof gate:** a differential operator-permutation harness ([differential.test.ts](differential.test.ts)) asserts the live incremental view ≡ a from-scratch rebuild across every operator × source-shape × mutation, wired into `npm test` (**84 cases** — +10 chained-sort permutations added with the C3 fix). A `KNOWN_FAILURES` registry (now **empty**) **fails if a listed case starts passing**, so each fix had to flip the gate. Design analysis in [.claude/array-contract-design.md](.claude/array-contract-design.md).
...
**C3 — closed.** The chained windowed sort is fully fixed. A bounded window rotation only changes *content* at fixed positions, so `ZAValue._window()` now emits it as content-stable `BU1`s (plus a tail-only `BR1A`/`BI0A` for a genuine size change) instead of a mid-window evict+insert pair whose interior splice + inconsistent intermediate state corrupted a downstream positional sort. Two latent bugs fell out and were fixed alongside: `AZValue.XU0` never set `isArr` (ascending sorts over arrays skipped index-shift bookkeeping), and `filter`'s predicate-flip path over an array emitted splice-semantics `BI0`/`BR1` for what is really a hole fill/remove (now `RowOperator.loop` emits `BF0`/`BH1` and `ZAValue` implements them). Verified across the full chained-sort family (`za-window→az-window`, `az-window→za-window`, `za-window→za-window`, `za→az` unbounded, `za-window→az-window→map`, `filter→za-window→az-window`). Change-stream consequence: a single-row window rotation now emits `update`s, not `remove`+`insert` (the `index.test.ts` `za` change-stream was updated to match the no-churn shape the batch path already produced).
...
| [C3](#c3) | Chained **windowed** sort (`za(col,n).az(col,n)`) surfaces stale **content** | Correctness · windowed-sort | Medium | ✅ Fixed — content-stable rotation emission (`ZAValue._window()`) + `AZValue.isArr` + `filter`/sort `BF0`/`BH1` |
...
- Change-stream consequence: a single-row window rotation now emits `update`s, not `remove`+`insert` (no `undefined` flash on a rotated child view) — matching the batch-window path; `index.test.ts`'s `za` change-stream was updated to match.
...
- **`sort` phantom `-1` position key (chained windowed sort)** — fixed: `oidx >= 0 && oidx < this.n` guard in `BR2`/`BU2`; test [operators/sort/sort.test.ts:188-200](operators/sort/sort.test.ts#L188-L200). (The *content* staleness behind it is now also fixed — see [C3](#c3): content-stable rotation emission.)
