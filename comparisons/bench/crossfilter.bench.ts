...
function buildSingle(initRows: any) {
  const state = initRows.map((r: any) => ({ ...r }))
  const cf = crossfilter(state)
  const dSpread = cf.dimension((d: any) => d.ask - d.bid)
  dSpread.filterRange([THRESHOLD + 1e-9, Infinity])
  return { cf, dSpread, state }
}

function buildDashboard(initRows: any) {
  const state = initRows.map((r: any) => ({ ...r }))
  const cf = crossfilter(state)
  const dSpread = cf.dimension((d: any) => d.ask - d.bid)
  dSpread.filterRange([THRESHOLD + 1e-9, Infinity])
  ...
  const sumCount = dAll.groupAll().reduce(
    (p: any, v: any) => { p.s += v.bid; p.n++; return p },
    (p: any, v: any) => { p.s -= v.bid; p.n--; return p },
    () => ({ s: 0, n: 0 }),
  )
  const avgBid = () => {
    const { s, n } = sumCount.value() as any
    return n ? s / n : 0
  }
  return { cf, dSpread, liquidCount, avgBid, state }
}

function tick(cf: any, state: any, t: any) {
