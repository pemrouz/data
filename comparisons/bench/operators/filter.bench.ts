// @ts-nocheck
// `filter` — per-operator comparison.
//
// Workload: 10_000 rows {id, val, active, ...}. Predicate: `val > 50`.
// Single tick: assign a fresh value to one row's `val`. Batch: TICK_COUNT
// ticks streamed back-to-back with the filtered result read after each.
//
// `data` runs the predicate only on the touched row (BU2 from the operator
// dispatch — see [operators/filter/index.ts](../../../operators/filter/index.ts)).
// Peers that wrap the array as a single signal walk all N rows on every tick.
...
const data: Variant = {
  ...
  run: async () => {
    ...
    const build = () => {
      ...
      const f = src.filter(pred)
      return { src, f }
    }
    const setup = measure(() => { build() })
    const single = (() => {
      const { src, f } = build(); void f[value]
      let i = 0
      return measure(() => {
...
const crossfilterV: Variant = {
  ...
  run: async () => {
    ...
    const single = (() => {
      const { cf, dVal, state } = build(); void dVal.top(Infinity)
      let i = 0
      return measure(() => {
...
const rxjsV: Variant = {
  ...
  run: async () => {
    ...
    const build = () => {
      ...
      const filtered$ = subj.pipe(map(rows => rows.filter(pred)))
...
const preactV: Variant = {
  ...
  run: async () => {
    ...
    const tickFn = (cells, t: Tick) => { cells[t.idx][t.field].value = t.value }
    const single = (() => {
      const { cells, filtered } = build()
      let i = 0
...
const svelteV: Variant = {
  ...
  run: async () => {
    ...
    const build = () => {
      ...
      const filtered = derived(store, rows => rows.filter(pred))
...
const op: OpBench = {
  operator: 'filter',
  notes: 'predicate `val > 50` over 10k rows; tick mutates one row',
