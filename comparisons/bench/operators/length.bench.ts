...
//
// data: `filter(d => d.active).length()`. The filter's BU2 emits one
...
const data: Variant = {
  ...
  run: async () => {
    ...
    const single = (() => {
      const { src, len } = build(); void len[value]
      let i = 0
      return measure(() => {
...
const crossfilterV: Variant = {
  ...
  run: async () => {
    ...
    const single = (() => {
      const { cf, count, state } = build(); void count.value()
      let i = 0
      return measure(() => {
