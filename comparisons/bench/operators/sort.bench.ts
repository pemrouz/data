...
const data: Variant = {
  ...
  run: async () => {
    ...
    const single = (() => {
      const { src, top }: any = build(); void top[value]
      let i = 0
      return measure(() => {
...
const crossfilterV: Variant = {
  ...
  run: async () => {
    ...
    const single = (() => {
      const { cf, dVal, state }: any = build(); void dVal.top(K)
      let i = 0
      return measure(() => {
