...
//
// Workload: 10_000 rows; whole-value projection `to((rows: any) => rows.length)`.
...
const op: OpBench = {
  ...
  notes: 'src.to((rows: any) => rows.length) over 10k rows; tick updates one val. crossfilter omitted — no whole-value projection primitive',
