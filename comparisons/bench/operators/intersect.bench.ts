...
const data: Variant = {
  ...
  run: async () => {
    ...
    const build = () => {
      ...
      const c = a.intersect(b as any)
      return { src, c }
    }
    const setup = measure(() => { build() })
    const single = (() => {
      const { src, c }: any = build(); void c[value]
      let i = 0
      return measure(() => {
...
const crossfilterV: Variant = {
  ...
  run: async () => {
    ...
    const single = (() => {
      const { cf, dActive, state }: any = build(); void dActive.top(Infinity)
      let i = 0
      return measure(() => {
