...
function applyProps(node, props2) {
  ...
  for (const k in props2) {
    ...
    if (k === "className" || k === "class") {
      ...
      node = node.id(v, true);
    } else if (/^on[A-Z]/.test(k) && typeof v === "function" && !v[view]) {
...
function h(tag, props2, ...children) {
  if (typeof tag === "function") {
    const norm = children.length === 0 ? void 0 : children.length === 1 ? children[0] : children;
    const merged = props2 ? { ...props2, children: props2.children ?? norm } : { children: norm };
  ...
  let hasRowFn = false;
  for (const c of flat) {
    if (typeof c === "function" && !c[view] && !c[NODE]) {
      hasRowFn = true;
      break;
    }
  }
...
export { $, For, Fragment, HTML, Operators, SVG, Sink, createOperator, h, jsx, jsxDEV, jsxs, reactive, render, value, view };
