// compat/v2-records.ts — the PERMANENT v2 record profile.
//
...
import { attachSettled, leafAt } from '../kernel/node.js'
                                                                     

const sclone = (v         ) => (v === undefined ? v : structuredClone(v))
const ident = (v         ) => v
...
export class V2RecordSink    {
           wantsOrder = true
                                             
                                          
                                 // local mirror of the node's order channel
                           
                                             // sclone (v2 parity) or identity (clone:false)

  constructor(node             , out                             , opts             = {}) {
    this.node = node
    this.out = out
    ...
    if (node.kind === 'scalar') {
      this.order = null
      this.out({ type: 'update', key: [], value: this.c((node       ).value()) })
      return
    }
...
export function materialize   (snap                , order                          )          {
...
export function connectRecords   (node             , arr                  , opts             = {})                     {
