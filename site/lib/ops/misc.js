// ops/misc.ts — the remaining scalar/effect/utility family:
...
import { ScalarNode } from './aggregate.js'
...
// share the sum/avg exclusion semantics exactly.
         ...
         class TrackedScalarNode     extends ScalarNode     {
  ...
  constructor(
    ...
    this.col = col
    const base = read ?? (col === undefined ? (r     ) => r : (r     ) => r?.[col])
...
export class MaxNode     extends TrackedScalarNode     {
                       
                        

  constructor(runtime         , parent              , col         ) {
    ...
    if (n !== undefined && this.stale !== true) {
      if (this.best === undefined || (n       ) > (this.best       )) this.best = n
    }
  }
...
export class MinNode     extends TrackedScalarNode     {
...
export class SomeNode     extends TrackedScalarNode     {
                                       

  constructor(runtime         , parent              , fn                      ) {
    super(runtime, parent, 'some', undefined, (r     ) => !!fn(r))
  }
...
export class EveryNode     extends TrackedScalarNode     {
                                       

  constructor(runtime         , parent              , fn                      ) {
    super(runtime, parent, 'every', undefined, (r     ) => !!fn(r))
  }
...
export class ReduceNode     extends ScalarNode     {
                                                     
                   

  constructor(runtime         , parent              , fn                                         , init     ) {
...
export class ReduceIncrementalNode     extends ScalarNode     {
  ...
  constructor(
    ...
    this.init = init
    let acc = this.seed()
    parent.each((k, row) => { acc = addFn(acc, row      , k) })
    this.acc = acc
...
export class DistinctNode    extends DataNode          {
                                 
                                   // source key → arrival counter
                                       // source key → projected value
                                    // source key → distinct key
                                            // distinct key → holder source keys
                                     // distinct key → exposed value
...
export class TapNode    extends DataNode    {
  constructor(runtime         , parent             , fn                        ) {
  ...
  settle()                        {
    return this.in0                          // pure passthrough
  }
}
...
export class ToValueNode     extends ScalarNode     {
...
export class KeysNode    extends DataNode         {
...
export class ValuesNode    extends DataNode    {
  constructor(runtime         , parent             ) {
...
export function max   (src             , col         )             {
  return new MaxNode(src.runtime, src, col)
}
export function min   (src             , col         )             {
...
function colPred   (fnOrCol                                )                      {
  if (typeof fnOrCol === 'string') {
    const col = fnOrCol
    return (r     ) => (r == null ? undefined : r[col])
  }
