...
const isIndexKey = (s        ) => IDX_RE.test(s) && Number(s) <= 4294967294 // 2^32 - 2
...
export class BucketNode       extends DataNode    {
  ...
  // ── membership bookkeeping ──────────────────────────────────────────────────
    ...
    if (this.count !== null) {
      this.count.set(bk, (this.count.get(bk)          ) - 1)
  ...
  // one O(B) object fill, no String() pass, no sort, no temp Map.
          build(bk        )    {
    if (this.counts) return { value: this.count .get(bk) ?? 0 }                
    const sk = this.skOf .get(bk) 
    const o                    = {}
    for (const s of sk.sorted) o[s] = sk.bySk.get(s)     
    return o                
  }
  ...
  settle(seq        , origin             )                        {
    ...
    for (const [bk, prev] of touched) {
      const emptied =
        this.count !== null ? (this.count.get(bk) ?? 0) === 0 : (this.members .get(bk)                  ).size === 0
      ...
      if (!liveNow) {
        ...
        if (wasLive) {
          this.view.delete(bk)
          out.push({ op: 'remove', key: bk, prev: prev      })
        }
        continue
      }
      ...
      if (!wasLive) {
        ...
        !this.sameBucket(prev     , next)
      ) {
        this.view.set(bk, next)
        if (sk !== undefined) sk.emittedSize = sk.sorted.length
        out.push({ op: 'update', key: bk, row: next, prev: prev     , path: [] })
      }
