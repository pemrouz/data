// ops/setops.ts — set algebra over the KEY domain: intersect / union /
...
import { DataNode, leafAt } from '../kernel/node.js'
...
function rootsOf(n               , out                     = new Set())                     {
...
export class SetOpNode    extends DataNode    {
                             
                               // output MEMBERSHIP (rows resolve via exposed() — M6 P4b)
                             // except only: OR of the *others* args' bits (bit i = parents[i])
                                    // parents share ≥1 root source
                                                               // settle scratch — reused per commit
                                                          // PRIMARY deltas this commit (pre-state reconstruction)
                                                                          // union only: per-parent deltas

  constructor(runtime         , variant            , primary             , others                        ) {
  ...
  snapshot()                 {
    ...
    const m = new Map           ()
    this.view.eachKey((k) => m.set(k, this.exposed(k)     ))
    return m
  }
  ...
  each(fn                               )       {
    ...
    this.view.eachKey((k) => fn(k, this.exposed(k)     ))
  }
  ...
  settle(seq        , origin             )                        {
    ...
    if ((this.inMore === null || this.inMore.length === 0) && b0.rows.length === 1) {
      ...
      const ui = this.variant === 'union' ? this.parents.indexOf(this.inFrom0               ) : 0
    ...
    const touched = (this.touchedScratch ??= new Map                     ())
    const pds = (this.pdScratch ??= new Map                     ())
    const uds = this.variant === 'union' ? (this.udScratch ??= new Map()) : null
    this.fold(b0, this.inFrom0               , touched, pds, uds)
    if (this.inMore !== null)
      for (const { from, batch } of this.inMore)
        this.fold(batch                  , from               , touched, pds, uds)
  ...
  // also its pre-state (height order: parents settled before us).
    ...
    return (this.parents[i]               ).hasRow(k)
  }

          preRowParent(i        , k        , di                         )                {
    if (di !== undefined) return di.op === 'add' ? undefined : di.prev
    return (this.parents[i]               ).rowAt(k)
  }
  ...
  // passes ui = -1).
    ...
    if (!preLive && postLive) {
      ...
      const prev = this.preExposed(k, pd, ui, ud)     
      ...
      const preRow = this.preExposed(k, pd, ui, ud)     
      const row = this.exposed(k)     
