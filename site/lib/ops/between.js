// ops/between.ts — the range-membership view (crossfilter's core).
//
...
export class BetweenNode    extends DataNode    {
                     
                     // APPLIED bounds (as of the last settle)
                    
                                        // TARGET bounds (post-write, read-your-writes)
                               // in-range MEMBERSHIP (rows delegate to parents[0] — M6 P1/P4)
  ...
  snapshot()                 {
    ...
    this.view.eachKey((k) => m.set(k, p.rowAt(k)     ))
    return m
  }
  ...
  each(fn                               )       {
    ...
    const p = this.parents[0]
    this.view.eachKey((k) => fn(k, p.rowAt(k)     ))
  }
  ...
  settle(seq        , origin             )                        {
    ...
    const pending = (this.pendScratch ??= new Map                     ())
    ...
    const rows =
      pending.size === 1 ? [pending.values().next().value               ] : [...pending.values()]
    pending.clear()
    this.view.maybeFlip() // re-polarize once per settle, never per key
    return { seq, origin, rows, order: undefined, scalar: undefined }
  }
...
export function between   (
  ...
  for (const el of bounds) {
    ...
    const handleLike =
      typeof el === 'object' && ((el       )[Symbol.for('data.v4.node')] !== undefined || el instanceof DataNode)
    throw new Error(
      handleLike
