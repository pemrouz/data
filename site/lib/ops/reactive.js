// ops/reactive.ts — the UNIFORM reactive value-slot binder (v3's answer to
...
import { DataNode, SourceNode } from '../kernel/node.js'
                                                   
import { registry } from './registry.js'
                                          
import { FilterNode } from './rowops.js'
import { OrderedView, cmpBy } from './ordered.js'
                                                 
import { SumNode, AvgNode } from './aggregate.js'
import { MaxNode, MinNode } from './misc.js'
import { between, BetweenNode } from './between.js'
...
export function reactiveArg(arg         )              {
...
export function bindParam   (owner               , arg         , onChange                )    {
...
function lastParam(batch                  , fallback         )          {
  let v = fallback
  for (const d of batch.rows) if (d.op !== 'remove') v = (d       ).row
  return v
}
...
function adoptParam(node               , paramSrc                 )       {
  ;(node.parents                   ).push(paramSrc)
...
function publishScalar(node     , seq        , origin             )                            {
...
export class CompareRNode    extends FilterNode    {
                              // APPLIED threshold (as of the last settle)
                                        // TARGET threshold (post-write)
  ...
  constructor(
    ...
    const box = { t: t0 }
    const cmp = CMP[op]
    super(runtime, parent, (row     ) => cmp(row?.[col], box.t), op)
    this.box = box
...
export function compareR   (src             , op       , col        , threshold         )                  {
  const ra = reactiveArg(threshold)
  const t0 = ra.current()
  const paramSrc = new SourceNode         (src.runtime, { [PKEY]: t0 }, `param:${op}`)
  const node = new CompareRNode   (src.runtime, src, paramSrc, op, col, t0)
  if (ra.isReactive) {
...
export class OrderedRNode    extends OrderedView    {
                                        // TARGET n (post-write); this.n = APPLIED
...
export function orderedR   (
  ...
  const node = new OrderedRNode   (src.runtime, src, paramSrc, name, cmpFor   (name, by), normN(n0))
  if (ra.isReactive) {
...
export class SumRNode    extends SumNode    {
                                       
                               

  constructor(runtime         , parent             , paramSrc                     , col0                    ) {
...
export class AvgRNode    extends AvgNode    {
...
export function avgR   (src             , col         )              {
  const ra = reactiveArg(col)
  const c0 = normCol(ra.current())
  const paramSrc = new SourceNode         (src.runtime, { [PKEY]: ra.current() }, 'param:avg')
  const node = new AvgRNode   (src.runtime, src, paramSrc, c0)
  if (ra.isReactive) {
    node.target = ra.current
    bindParam(node, col, (v) => paramSrc.write(PKEY, [], v))
  }
...
function reprojected(node     , col                    )       {
  node.col = col
  const base = col === undefined ? (r     ) => r : (r     ) => r?.[col]
  node.projFn = (row     ) => {
    const x = base(row)
    return x === undefined || x === null ? undefined : x
  }
...
function extremumValue(node     )          {
  ...
  try {
    return node.recompute(node.parents[0].snapshot()                        )
  } finally {
    node.col = savedCol
...
export class MaxRNode    extends MaxNode    {
...
export class MinRNode    extends MinNode    {
...
export function maxR   (src             , col         )              {
  const ra = reactiveArg(col)
  const paramSrc = new SourceNode         (src.runtime, { [PKEY]: ra.current() }, 'param:max')
  const node = new MaxRNode   (src.runtime, src, paramSrc, normCol(ra.current()))
  if (ra.isReactive) {
    node.target = ra.current
    bindParam(node, col, (v) => paramSrc.write(PKEY, [], v))
  }
...
export function minR   (src             , col         )              {
  const ra = reactiveArg(col)
  const paramSrc = new SourceNode         (src.runtime, { [PKEY]: ra.current() }, 'param:min')
  const node = new MinRNode   (src.runtime, src, paramSrc, normCol(ra.current()))
  if (ra.isReactive) {
    node.target = ra.current
    bindParam(node, col, (v) => paramSrc.write(PKEY, [], v))
  }
...
export function betweenR   (src             , col        , arg         )                 {
  const ra = reactiveArg(arg)
  const node = between(src, col, normBounds(ra.current()))
  bindParam(node, arg, (b         ) => node.setBounds(normBounds(b)))
  return node
}
...
export function installReactive()       {
  if (installed) return
  installed = true
