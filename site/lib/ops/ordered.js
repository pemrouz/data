// ops/ordered.ts — the ORDERED view family: az/za (column or comparator,
...
export class OrderedView    extends DataNode    {
                                   
                    // window size; Infinity = unbounded
                         // tiebreak direction: 1 = arrival order, -1 = newest-first (reverse)
                               // ALL live parent rows (current references)
                                  
                                                     // prev rows of touched keys — ACTIVE only during phase-B removals // key → insertion seq (stable tiebreak)
                        
                            // full order over ALL parent rows
                           // materialized: first min(n, size) ranked keys
                              // membership of MY view (the window)

  constructor(runtime         , parent             , name        , cmp                  , n         , tieDir         = 1) {
    ...
    this.index = new OrderIndex((a, b) => {
      const c = this.userCmp(this.rowOf(a)     , this.rowOf(b)     )
      return c !== 0 ? c : ((this.tie.get(a)          ) - (this.tie.get(b)          )) * this.tieDir
    })
    if (cmp === ARRIVAL) {
  ...
  snapshot()                 {
    if (this.runtime.midBatch) {
      ...
      const m = new Map           ()
      for (const k of keys) m.set(k, rows.get(k)     )
      return m
    }
    const m = new Map           ()
    const p = this.parents[0]               
    for (const k of this.window) m.set(k, p.rowAt(k)     )
    return m
  }
  ...
  each(fn                               )       {
    ...
    const p = this.parents[0]               
    for (const k of this.window) fn(k, p.rowAt(k)     )
  }
  ...
  rowAt(key        )                {
    ...
    return this.winSet.has(key) ? (this.parents[0]               ).rowAt(key) : undefined
  }
  ...
  // every known key (matching what settle will assign).
    ...
    const keys = [...snap.keys()].sort((a, b) => {
      ...
      return c !== 0 ? c : ((tmpTie.get(a)          ) - (tmpTie.get(b)          )) * this.tieDir
    })
  ...
  settle(seq        , origin             )                        {
    ...
    const dmap = new Map                     ()
    const toInsert           = []
    const removedIdx           = [] // keys leaving the index (removes + re-ranks)
    ...
    const pendingDel           = [] // removes: tie cleanup deferred past index ops
    ...
    for (const d of input.rows                          ) {
      ...
      switch (d.op) {
        ...
        case 'update': {
          ...
          if (this.userCmp(d.prev     , d.row) !== 0) {
            removedIdx.push(d.key)
            overlay.set(d.key, d.prev)
            toInsert.push(d.key)
          }
    ...
    for (const k of preWindow) {
      ...
      const prev = d !== undefined && d.op !== 'add' ? d.prev : ((this.parents[0]               ).rowAt(k)     )
      out.push({ op: 'remove', key: k, prev })
    }
    for (const k of newWindow) {
      if (!preSet.has(k)) {
        out.push({ op: 'add', key: k, row: ((this.parents[0]               ).rowAt(k))      })
      } else {
        const d = dmap.get(k)
...
export function cmpBy   (proj                     , dir        )                   {
  return (a, b) => {
    const va = proj(a)       
    const vb = proj(b)       
    const ba = isBad(va)
