// render/builders.ts — the HTML.*/SVG.* builder DSL (M4.5b, sugar layer).
//
...
import { el, text, bind } from './index.js'
...
function isPropsObject(x         )          {
  ...
  if ((x       )[NODE] instanceof DataNode) return false // $ handle — checked before any .kind probe
...
export function normChildren(children           )          {
  const out          = []
  const push = (c         )       => {
...
const EMPTY_DOT           = { classes: [], id: null, attrs: {} }
...
export const HTML                   = namespaceProxy()
export const SVG                   = namespaceProxy()
