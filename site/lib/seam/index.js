...
import { SourceNode, DataNode, attachSettled, leafAt } from '../kernel/node.js'
                                                           
                                                   
import { currentScope } from '../kernel/scope.js'
import { SCHEMA_VERSION, RESERVED } from '../contract/index.js'
...
function applyV2(src                 , r                )       {
  if (r.type === 'move') {
    ...
    const k = ord[(r       ).from          ]
    if (k !== undefined) src.move(k, (r       ).to          )
    return
  }
  if (r.type === 'insert') {
...
function applyV2WholeValue(src                 , value         )       {
  const ord = src.currentOrder()
  if (ord !== null) {
...
export const HOT = { update: 0, insert: 1, remove: 2 }          // ≡ fero kernel/env.ts REC
...
function applyHot(src                 , r           )       {
  ...
  if (r.type === HOT.insert) {
    ...
    src.write(key          , EMPTY_PATH, r.value)
    return
  }
  throw new Error(`data: lane() unknown record type ${r.type}`)
}

export function lane(target              , opts             = {})          {
  const src = resolveSource(target)
  if (src.ordered)
    ...
    const run = () => {
      for (let i = 0; i < records.length; i++) {
        try {
          ...
          ;(rejects ??= []).push({ index: i, record: records[i]                           , error: e })
        }
      }
    }
    let flushError          = NO_FLUSH_ERROR
...
export function fromAsync   (
  ...
  const source = new SourceNode   (runtime, (keyFn ? {} : [])       , 'fromAsync')
  let status              = 'pending'
  let error         
  let cancelled = false
  let buffer             = null
...
export class InMemoryBacking                                {
                                        

  constructor(runtime         , value                         , name = 'backed-source') {
    ;(this                             ).source = new SourceNode   (runtime, value, name)
  }
...
export function mount   (runtime         , backing                  )                 {
  let src                       = null
  const sub = backing.subscribe({
    ...
    init(snapshot, order) {
      ...
      src = new SourceNode   (runtime, {}       , 'mounted')
      runtime.batch(() => {
