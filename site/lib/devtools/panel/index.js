// devtools/panel/index.ts — the panel shell: host element + closed shadow
...
import { createCtx, registerRefreshable } from './ctx.js'
...
export function mountPanel(opts                     = {})              {
  ...
  const maxWidth = ()         =>
    typeof window !== 'undefined' && typeof (window       ).innerWidth === 'number'
      ? Math.max(WIDTH_MIN, Math.floor((window       ).innerWidth * WIDTH_MAX_FRAC))
      : 1600
  ...
  const ctx           = createCtx(defaultRuntime(), dock, doc)
