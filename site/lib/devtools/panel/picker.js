// devtools/panel/picker.ts — the DOM picker (◎) + Alt-hover badge.
//
...
import { fromDOM, rowElements } from '../dom.js'
...
const isAlt = (e     )          => e.key === 'Alt' || e.code === 'AltLeft' || e.code === 'AltRight'

export function mountPicker(ctx          , toolbar     )                      {
  const doc = ctx.doc
  ...
  const onBtn = ()       => { picking ? stopPicking() : startPicking() }
