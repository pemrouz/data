...
export class Store    {
                          ...
                          // synthetic key counter (array-born rows)
                       
                                        // adopted lane — null once promoted
                                 // adopted iteration order (Object.keys at adopt)
                         // adopted-ARRAY lane: key i ≡ slot i, no keySlot/slotKey
  ...
  static adoptObject   (obj                   )           {
    ...
    return st
          ...
          okeysOf()           {
    return (this.okeys ??= Object.keys(this.obj                     ))
  }
  ...
  get(key        )                {
    if (this.ident) return this.identHit(key) ? this.slots[key          ] : undefined
    if (this.obj !== null)
  ...
  updateLive(key        , row   )       {
    ...
    this.slots[this.keySlot.get(key)          ] = row
  }

  // Single-lookup accessor for the write hot path (has + get in one hash).
  slotOf(key        )                     {
    if (this.ident) return this.identHit(key) ? (key          ) : undefined
  ...
  *entries()                                {
    if (this.ident) {
      for (let i = 0; i < this.slots.length; i++) yield [i, this.slots[i]     ]
      return
    }
    if (this.obj !== null) {
  ...
  *keys()                           {
  ...
  each(fn                               )       {
    if (this.ident) {
      ...
      for (let i = 0; i < slots.length; i++) fn(i, slots[i]     )
      return
    }
    if (this.obj !== null) {
      const okeys = this.okeysOf()
    ...
    for (const [k, s] of this.keySlot) fn(k, this.slots[s]     )
  }

  snapshot()                 {
    const m = new Map           ()
    if (this.ident) {
      for (let i = 0; i < this.slots.length; i++) m.set(i, this.slots[i]     )
      return m
    }
    if (this.obj !== null) {
      const okeys = this.okeysOf()
