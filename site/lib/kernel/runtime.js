// kernel/runtime.ts — the commit machinery. SCHEDULE.md made executable.
//
...
export class Runtime {
  seq = 0
          batchDepth = 0
          flushing = false
          draining = false // executing ONE queued re-entrant write (apply directly)
          dirty                    = [] // array + per-source inDirty flag: no Set hashing/iterator on the hot path
          queue                                                  = [] // re-entrant writes → next commit(s), FIFO (origin captured at issue time)
          currentOrigin                     = null
          defaultOrigin              = Symbol('user')
          hooks = new Set                         ()
          registry = new Set                        ()
