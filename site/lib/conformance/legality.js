// conformance/legality.ts — the delta legality checker.
//
...
export class LegalityChecker    {
  live = new Set        ()
  order                  = null // tracked only if the node emits order deltas
  lastSeq = -1
           node        
  constructor(node        ) {
    this.node = node
  ...
  apply(batch                )       {
    ...
    for (const d of rows) {
      ...
      switch (d.op) {
        ...
        default: {
          const never        = d
          throw new LegalityError(this.node, seq, `unknown row op ${(never               ).op}`)
        }
      }
    }
