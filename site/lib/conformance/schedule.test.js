// conformance/schedule.test.ts — SCHEDULE.md made EXECUTABLE (W4).
//
...
import { test } from '../../shim/test.js'
import assert from '../../shim/assert.js'
import { Runtime } from '../kernel/runtime.js'
import { SourceNode } from '../kernel/node.js'
import { conform } from './harness.js'
import { filter } from '../ops/rowops.js'
import { sum } from '../ops/aggregate.js'
import { intersect } from '../ops/setops.js'
import { SCHEDULE_VERSION } from '../contract/index.js'
import { handleFor } from '../api/index.js'
                                                       
import { ingest, fromAsync, InMemoryBacking } from '../seam/index.js'
...
function capture   (node     , origin                = null)                   {
  const out                   = []
  node.connect({ wantsOrder: true, origin, apply: (b                ) => out.push(b) })
  return out
}
...
test('clause 1: bare write = synchronous batch of one; Object.is-equal write emits nothing', () => {
  ...
  const src = new SourceNode     (rt, { a: { n: 1 } })
  conform(src)
  const batches = capture     (src)
...
test('clause 2: mid-batch reads see all writes so far; no effect fires until batch close', () => {
  const rt = new Runtime()
  const src = new SourceNode     (rt, { a: { n: 1 } })
  const big = filter(src, (r     ) => r.n > 10)
  conform(src)
  conform(big)
  const srcBatches = capture     (src)
  const bigBatches = capture     (big)
...
test('clause 3: height-ordered propagation, exactly once, parents settled before a child sink fires', () => {
  const rt = new Runtime()
  const src = new SourceNode     (rt, { a: { n: 5 }, b: { n: 20 } })
  const lo = filter(src, (r     ) => r.n < 100)
  const hi = filter(src, (r     ) => r.n > 1)
...
test('clause 4: a throwing effect never starves siblings; failures collect into one AggregateError', () => {
  ...
  src.connect({ wantsOrder: false, origin: null, apply: (b                  ) => seen.push(b.seq) })

  assert.throws(
    () => src.write('a', [], { n: 3 }),
    (e         ) => e instanceof AggregateError && /1 effect\(s\) failed during commit/.test((e         ).message),
  )
...
test('clause 5: an unbounded write cascade throws at the cycle cap, not an infinite loop', () => {
  const rt = new Runtime()
  const src = new SourceNode               (rt, { k: { n: 0 } })
  let i = 0
  src.connect({
    wantsOrder: false,
    origin: null,
...
test('clause 6: same-origin sinks are suppressed (source AND derived); re-entrant writes carry issue-time origin', () => {
  ...
  const view = filter(src, (r     ) => r.n > 0)

  const asMine = capture     (src, mine)
  const asOther = capture     (src, Symbol('peer-B'))
  const asNull = capture     (src, null)
  const derivedAsMine = capture     (view, mine)
...
test('clause 7: sink()/connect() attached INSIDE batch() — init is the settled commit, apply starts at the NEXT commit', () => {
  ...
  rt.batch(() => {
    ...
    h.sink({
      init: (s                   ) => (snap = new Map(s)),
      apply: (b         ) => applies.push(b),
    })
...
test('clause 8: A→B→A and add+remove annihilate; multi-write batches consolidate to one delta per key', () => {
  ...
  ok(a.op === 'update' && (a       ).prev.n === 1) // first prev, last row
})
...
test('clause 10b: every absent-target remove is a silent no-op, never a throw or a write', () => {
  const rt = new Runtime()
  const src = new SourceNode     (rt, { r: { a: { b: 1 } }, s: { flat: 0 }, u: { x: undefined } })
  conform(src)
  const batches = capture     (src)
...
test('clause 10c: vivify-under-null/scalar on live rows; deep write to a non-live key throws', () => {
  const rt = new Runtime()
  const src = new SourceNode     (rt, { r: { a: null, s: 5 } })
  conform(src)
...
test('clause 8: same-batch field-write/field-delete merges — annihilate, keep-delete, drop-marker, restore', () => {
  const rt = new Runtime()
  const src = new SourceNode     (rt, { a: { n: 1 }, b: { n: 2 }, c: { n: 3 }, d: { n: 4 } })
  conform(src)
  const batches = capture     (src)
...
test('clause 10d: a poison record never aborts its siblings; rejects collect into one AggregateError', () => {
  ...
  assert.throws(
    ...
    (e         ) => e instanceof AggregateError && /rejected 1 of 3/.test((e         ).message),
  )
...
test('clause 9: coalescing changes commit COUNT, never semantics — default stays sync per chunk', async () => {
  ...
  const syncBatches = capture               (sync.source)
  ...
  const coBatches = capture               (co.source)
  await new Promise((r) => setTimeout(r, 20))
...
test('clause 11: NUL/unicode keys are total; undefined/NaN are first-class; by-ref values are shared-immutable', () => {
  ...
  src.write('nan', [], { x: NaN })
  ok(Number.isNaN((src.rowAt('nan')       ).x)) // in-engine NaN survives (wire codecs own their fate)

  // By-ref surfaces share the SAME reference (the shared-immutable contract).
  const row = { deep: { v: 1 } }
  let seen     
  src.connect({ wantsOrder: false, origin: null, apply: (b     ) => (seen = b.rows[0]?.row) })
  src.write('r', [], row)
  ok(seen === row)
})
