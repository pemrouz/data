# Notification protocol & architecture reference

The complete reference for `data`'s reactive engine — the View / Sink / notification **protocol** (the terse code legend, when each verb fires, propagation rules, and the array-source shift contract), plus how the pieces fit together. Read this if you're working on the library internals (anything in [core.ts](core.ts), [row.ts](row.ts), or implementing an operator) or you simply want to understand how one mutation flows through the derived views down to the DOM. For the public API and usage, see [README.md](README.md); for the Claude-session working conventions, see [CLAUDE.md](CLAUDE.md).

> This file is the **canonical** source for the notification protocol. The legend in [CLAUDE.md](CLAUDE.md#notification-code-legend-quick-reference) is a quick-reference subset; [.claude/architecture.md](.claude/architecture.md) is now just a pointer here. Keep them in sync per the convention at the bottom of this file.
...
- **`ZAValue`/`AZValue` and `BetweenValue`** cache `isArr` in `XU0` and route `BR1`/`BI0` through array-aware paths that decrement every `sorted` key above each removed position (or increment for non-end inserts), splice the sparse `view.value` in lockstep, and refill the visible window from the post-shift keys. (`AZValue` overrides `XU0`, so it must set `isArr` too — a 2026-06-06 fix; without it an ascending sort over an array skipped the shift and mis-tracked positions on a mid-array insert/remove.)
- **`ZAValue._window()`** reconciles a **bounded** window after a rotation as content-stable `BU1`s (a row crossing the window boundary only changes *content* at fixed positions) plus a tail-only `BR1A`/`BI0A` for a genuine size change — never a mid-window evict+insert pair. Mid-window splices corrupt a downstream positional sort's position→rank map (chained windowed sort); tail splices + `BU1`s compose correctly. The bounded `BU1` boundary-cross, `BI0` full-window insert, and `BR1`/`BR1A` in-window removal all route through it; unbounded sorts keep genuine mid-array splices. In-window rank shuffles still emit a single `BMV1`. Sort also implements `BH1`/`BF0` (rank in/out with no index shift) so a `filter→windowed-sort` chain over an array — where filter's predicate flip emits a hole — stays consistent.
...
- `DOMSink` ([render/index.ts:11](render/index.ts#L11)) is a regular sink — implements `XU0` / `BU1` / etc. to create/remove/reorder DOM nodes. Dense arrays use a tail-relative `create_node`/`remove_node`; a **sparse** array (a `between`/`intersect`/`union`/`except` view bound straight to a row template) is reconciled **index-keyed** (`_reconcile_sparse`) and maintained via `BH1`/`BF0` (`_remove_at`/`_create_at`), so excluded slots leave no phantom row.
...
This is the canonical protocol reference. If you change the notification scheme, sink contract, propagation rules, or operator dedup behavior, update the affected sections of this file in the same commit. If you add a new sink type or a new notification code, extend the relevant table — and mirror the change into the quick-reference legend in [CLAUDE.md](CLAUDE.md#notification-code-legend-quick-reference) so the two don't drift.
