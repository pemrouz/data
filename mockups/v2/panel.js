// data devtools v2 — working prototype panel.
//
// Goal of this module: prove that the chosen UX (graph-first edge-anchored
// dock + slide-in inspector + Alt-hover badges) can be driven entirely by
// the public devtools API exposed in devtools/index.ts ($.graph, $.inspect,
// $.trace, $.profile, $.fromDOM, $.cascades). When this prototype is
// promoted to devtools/panel/, no library API has to change — the migration
// is a shell rewrite, not a core rewrite.
//
// Pure DOM, no framework dependency. The host is a closed shadow root so
// page CSS can't bleed in.
import { $, value, view } from 'data/full'

// $.graph / $.inspect / $.trace / $.profile are attached when 'data/devtools'
// is imported (page does this before mounting us).

const TABS = ['inspect', 'events', 'profile']

export function mountPanelV2({ rootProxy }) {
  const host = document.createElement('div')
  host.className = '__ripple_panel_v2_host'
  ...
  const domPicker = createDomPicker(root, (el) => {
    ...
    // silently fail to open. See devtools-v2 picker regression test.
    const liveView = proxy[view]
    // Find the matching graph node from a fresh walk (rerenderGraph would
    // also work, but we need the node object for the inspector).
    const match = findNodeByView(walkGraph(rootProxy), liveView)
    if (!match) return
...
function createAltHover(panelRoot, panelHost) {
  ...
  const layer = document.createElement('div'); layer.className = '__rp_v2_alt_layer'
  document.body.appendChild(layer)
  const popover = document.createElement('div'); popover.className = '__rp_v2_alt_pop'; popover.hidden = true
  document.body.appendChild(popover)
  const layerStyle = document.createElement('style')
  layerStyle.textContent = `
    .__rp_v2_alt_layer { position: fixed; inset: 0; pointer-events: none; z-index: 2147483600; }
    .__rp_v2_alt_pop {
    ...
    .__rp_v2_alt_pop.pinned { border-color: #9bb3e3; pointer-events: auto; }
    .__rp_v2_alt_pop .h {
    ...
    .__rp_v2_alt_pop .h .x:hover { color: #e6e6e6; border-color: #9be3a8; }
    .__rp_v2_alt_pop dl { display: grid; grid-template-columns: 50px 1fr; gap: 2px 8px; margin: 0 0 4px; }
    .__rp_v2_alt_pop dt { color: #888; }
    .__rp_v2_alt_pop dd { margin: 0; color: #ddd; }
    .__rp_v2_alt_pop .hint { color: #666; font-size: 10px; }
    .__rp_v2_alt_badge {
      position: absolute;
    ...
    .__rp_v2_alt_outline { outline: 1px dashed #9be3a8; outline-offset: 1px; }
    .__rp_v2_alt_hovered { outline: 2px solid #9be3a8 !important; outline-offset: 1px; }
  `
  ...
  function renderBadges() {
    ...
    for (const el of gatherTargets()) {
      el.classList.add('__rp_v2_alt_outline')
      ...
      badge.className = '__rp_v2_alt_badge'
  ...
  function clear() {
    layer.innerHTML = ''
    document.querySelectorAll('.__rp_v2_alt_outline').forEach(e => e.classList.remove('__rp_v2_alt_outline'))
    document.querySelectorAll('.__rp_v2_alt_hovered').forEach(e => e.classList.remove('__rp_v2_alt_hovered'))
  ...
  const onMove = (e) => {
    ...
    document.querySelectorAll('.__rp_v2_alt_hovered').forEach(x => x.classList.remove('__rp_v2_alt_hovered'))
    if (t) {
      t.classList.add('__rp_v2_alt_hovered')
      current = t
