...
- The landing site ([index.html](index.html)) is a from-scratch v2: an all-monospace editorial page whose centrepiece is a live `data`-vs-MobX order-book race ([assets/race.js](assets/race.js)). Page assets: [assets/v2.css](assets/v2.css) (editorial system), [assets/feed.js](assets/feed.js) (one shared `$(trades)` feed + stream + the "updates since you arrived" counter), [assets/demos.js](assets/demos.js) (three live operator demos — filter/za/group — off that feed), [assets/landing.js](assets/landing.js) (bootstrap: wires the race, demos, copy, lazy devtools mount, counter, highlighter). The old multi-section landing (and the `examples/ticker/` insert-driven comparison demo, whose honest insert-rate crossover undersold `data`) were retired. [comparisons.html](comparisons.html) is the deeper appendix and still uses [assets/landing.css](assets/landing.css) + [assets/comparisons.css](assets/comparisons.css).
