...
- `npm run bench:compare` runs the cross-library comparison harness in [comparisons/bench/](comparisons/bench/) (this lib vs. crossfilter / MobX / RxJS / Solid / Preact / Vue / Svelte / React) on the canonical `map → filter → top-K` workload. `npm run bench:ops` runs the **per-operator** comparison harness in [comparisons/bench/operators/](comparisons/bench/operators/), exercising each operator in isolation against the same peer set and printing one markdown table per operator plus a regressions section flagging cases where `data` isn't fastest. `BENCH_OPS=filter,map npm run bench:ops` runs a subset. Both **report**, they do not gate — peer regressions don't fail this repo's CI. The two in-browser comparison demos ([examples/multidim/](examples/multidim/), [examples/trades/](examples/trades/)) run the same workload across the same nine libraries with live per-row reactive-cost trackers. The landing page ([index.html](index.html)) is now the comparison surface itself: a peer-switchable `data`-vs-peer order-book race ([assets/race.js](assets/race.js)) that reuses the `examples/trades/lib-*.js` implementations, plus a workload toggle that lazy-loads the full `examples/multidim/` demo in an iframe. Peers load from esm.sh via `index.html`'s importmap — if you bump peer versions, bump them there and in the example importmaps. (The standalone `comparisons.html` + `assets/landing.css` + `assets/comparisons.css` were removed once the landing page absorbed them.)
...
- The landing site ([index.html](index.html)) is a from-scratch v2: a single all-monospace editorial page that absorbed the old separate comparisons page. Centrepiece is a configurable live race ([assets/race.js](assets/race.js)) — a **peer picker** (data vs any of nine libs, reusing `examples/trades/lib-*.js` + `gen.js` + `views.js`) and a **workload toggle** (in-page streaming order book; or the full 231k-row [examples/multidim/](examples/multidim/) brushing demo lazy-loaded in an iframe). Other page assets: [assets/v2.css](assets/v2.css) (editorial system + the reused order-book card styles), [assets/feed.js](assets/feed.js) (one shared `$(trades)` feed + stream + the "updates since you arrived" counter), [assets/demos.js](assets/demos.js) (three live operator demos — filter/za/group — off that feed), [assets/landing.js](assets/landing.js) (bootstrap: wires the race, demos, copy, lazy devtools mount, counter, highlighter). Operator perf is inlined as a catalogue table linking each op to its `operators/<op>/BENCHMARK.md`. Retired along the way: the first dot-grid v2 attempt (`hero.js`/`operators.js`), the `examples/ticker/` demo (its insert-rate crossover undersold `data`), and the standalone `comparisons.html` + `landing.css` + `comparisons.css`.
