...
| `data/devtools`         | [devtools/index.ts](devtools/index.ts)        | Opt-in inspection helpers — importing this attaches `$.inspect`, `$.graph`, `$.fromDOM`, `$.highlight`, `$.trace`, `$.profile`, `$.cascades` onto the canonical `$`, AND lazy-loads + auto-mounts an in-page overlay panel ([devtools/panel/](devtools/panel/)) with Graph (tree or DAG) / Events / Profile / Flame / Replay tabs, a DOM picker, and a hover inspector. Append `?nopanel` to the URL to suppress the panel; `$.devtools.panel.{open,close}()` for explicit control. |
...
- `npm run bench:compare` runs the cross-library comparison harness in [comparisons/bench/](comparisons/bench/) (this lib vs. crossfilter / MobX / RxJS / Solid / Preact / Vue / Svelte). It **reports**, it does not gate — peer regressions don't fail this repo's CI. The synthetic numbers it produced used to live in [comparisons.html](comparisons.html); that page now points at the two in-browser comparison demos ([examples/multidim/](examples/multidim/), [examples/ticker/](examples/ticker/)) instead, which run the same workload across the same nine libraries with live per-row reactive-cost trackers. If you bump peer versions, also bump them in the importmaps of both example pages.
...
- [examples/ticker/](examples/ticker/) — insert-driven counterpart. A synthetic market-data tape at 1k–100k ticks/s, fed identically to the same nine libraries. Each row maintains top movers (top-5 by % change) and sector volume (rolling sum over a 500k-tick window) using its own reactive primitives. Per-row CPU tracker sums each lib's ingest call time plus its render callback time so ordering of libs in the loop doesn't bias the comparison ([examples/ticker/latency.js](examples/ticker/latency.js) carries the rationale). data's canonical row uses `za('pctChg', 5)` over a `$({})` of per-symbol LWW state plus `reduce(add, remove, init)` over a counter-keyed rolling window — both O(Δ) per tick, where peer libs walk O(WINDOW) per render. Same `!examples/ticker/**/*.js` ignore rule for the source files. No smoke test yet (consider adding one alongside `tests/multidim.spec.ts`).
...
- Operator dedup is opt-in via a `matches()` method. `between` ([operators/between/index.ts:6](operators/between/index.ts#L6)), `sort`/`za`/`az`/`top` ([operators/sort/index.ts:6](operators/sort/index.ts#L6)), `intersect` ([operators/intersect/index.ts](operators/intersect/index.ts)), the scalar aggregates `sum`/`avg`/`max`/`min`/`some`/`every` ([operators/aggregate/index.ts](operators/aggregate/index.ts)), `distinct` ([operators/distinct/index.ts](operators/distinct/index.ts)), and `reduce` ([operators/reduce/index.ts](operators/reduce/index.ts)) implement it; repeated calls with identical args return the cached view. `filter`, `map`, `length`, `group`, `to`, `tap`, `union`, `except`, `keys`, `values`, `reverse` create a fresh operator on every call.
