...
| `data/devtools`         | [devtools/index.ts](devtools/index.ts)        | Opt-in inspection helpers — importing this attaches `$.inspect`, `$.graph`, `$.fromDOM`, `$.highlight`, `$.trace`, `$.profile`, `$.cascades` onto the canonical `$`, AND lazy-loads + auto-mounts an in-page overlay panel ([devtools/panel/](devtools/panel/)) with Graph / Events / Profile / Flame tabs and a DOM picker. Append `?nopanel` to the URL to suppress the panel; `$.devtools.panel.{open,close}()` for explicit control. |
