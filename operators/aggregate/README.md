...
- **Dedup** — all six implement `matches(col)` ([index.ts:47](index.ts#L47)), so `proxy.sum('p')` (or `proxy.some(fn)`) twice with the same argument returns the *cached* view; the dedup key is the `col` string / `fn` reference. Different column → different view. (`some`/`every` dedup on the *same fn reference* — an inline arrow each call is a fresh predicate and won't hit the cache.)
