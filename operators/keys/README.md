...
- **Excluded slots are skipped on the append path.** A sparse `BI0` pair whose value is `undefined` (the explicit-`undefined` slots `between`/`intersect`/`union`/`except` leave) is not pushed — but note a full rebuild reflects whatever `Object.keys`/`Object.values` sees on the source, including enumerable `undefined` slots. Densify upstream if you want those gone (`vp.to(arr => arr.filter(r => r !== undefined))`).
