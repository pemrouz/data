...
1. **`index.ts`** — operator class(es) + standalone factory.
   ...
   - Export the class(es) and a standalone factory, **typed to return a `Data<…>`** so `opName(src, …)[value]` reads and chains (mirror the method-style return: `Data<T>` for a row-preserving op, `Data<number>`/`Data<boolean>`/`Data<RowOf<T>[]>`/… for a derived/scalar one). Infer `T` from the source; leave the other args loose (`any`): `import type { Data } from '../../core.ts'` then `export const opName = <T>(source: Data<T>, ...args: any[]): Data<T> => createOperator(source, OpClass, ...args)`. (`createOperator` returns `any`, which lets the precise `Data<T>` return flow without a per-factory cast — see [../core.ts](../core.ts).)
