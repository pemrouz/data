...
spec({ op:'map', guarantee:'Fidelity', trigger:'insert/remove', shape:'object', asserts:'the projection tracks inserts, edits and removes through the change stream' }, async () => {
  // `res` stays `any`: this spec exercises OFF-SHAPE mutations a precise type
  // rejects — inserting into a child (`res[2].insert(5, 'num')`), deleting a field
  // (`delete res[1].num`, which also makes `d.num * 10` arithmetic on a possibly-
  // missing field), adding dynamic keys, and clearing the whole value slot.
  const res: any = $({ 0: { num: 1 }, 1: { num: 2 }, 2: { num: 3 } })
  const mapped = map(res, (d: any) => d.num * 10)
